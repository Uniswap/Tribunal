...
contract TribunalTest is Test {
    ...
    function test_fillRevertsOnInvalidTargetBlock() public {
        ...
        Fill memory mandate = Fill({
            ...
            scalingFactor: 0,
            priceCurve: emptyDecayCurve,
            recipientCallback: new RecipientCallback[](0),
            salt: bytes32(uint256(1))
        });

        Lock[] memory commitments = new Lock[](1);
        commitments[0].token = address(1);
        commitments[0].amount = 1 ether;
    ...
    function test_FillWithReentrancyAttack() public {
        ...
        Mandate memory reentrantMandate = reentrantReceiver.getMandate();
        vm.expectCall(
            address(tribunal),
